...
describe("landscapes set unit test", () => {
    ...
    const landscape = { url: new node_url_1.URL(`https://my.landscape-1.com`).toString() };
    it("cmdLandscapeSet, confirmed added", async () => {
        ...
        (0, chai_1.expect)(lands.find((l) => l.url === landscape.url)).be.deep.equal(landscape);
    });
    ...
    it("cmdLandscapeSet, canceled", async () => {
        ...
        await cmdLandscapeSetProxy();
        (0, chai_1.expect)(lands.find((l) => l.url === landscape.url)).be.undefined;
    });
});
