...
describe("auth-utils unit test", () => {
    ...
    it("getJwt, iasjwt exists", async () => {
        ...
        mockAuth
            ...
            .withExactArgs(authProvider_1.BasRemoteAuthenticationProvider.id, [landscape])
            .resolves(session);
        (0, chai_1.expect)(await authUtilsProxy.getJwt(landscape, true)).to.be.equal(session.iasToken);
    });
    ...
    describe(`ext-login unit test for "mac"`, () => {
        ...
        it("retrieveJwt, login suceedded", async () => {
            ...
            (0, chai_1.expect)(await authUtilsProxy.retrieveJwt(landscape)).to.be.deep.equal({
                jwt: "token",
                iasjwt: "",
            });
        });
