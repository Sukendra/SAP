...
describe("authProvider unit test", () => {
    ...
    describe("getTokenByScope method", () => {
        ...
        it("getTokenByScope, objToken empty object, scope empty", () => {
            const instance = new BasRemoteAuthenticationProviderProxy(proxySecretSorage);
            (0, chai_1.expect)(instance[`getTokenByScope`](`{}`, [])).to.be.deep.equal({
                jwt: dummyToken,
            });
        });
        ...
        it("getTokenByScope, objToken provided, scope specified", () => {
            const instance = new BasRemoteAuthenticationProviderProxy(proxySecretSorage);
            (0, chai_1.expect)(instance[`getTokenByScope`](JSON.stringify(objToken), [landscape2])).to.be.deep.equal({ jwt: objToken[landscape2].jwt });
        });
    ...
    describe("checkForUpdates method", () => {
        ...
        it("checkForUpdates, scope specified, added scope", async () => {
            ...
            const args = stubEmitter.args[0][0];
            (0, chai_1.expect)(args.added.length).to.be.equal(1);
            (0, chai_1.expect)(args.added[0].accessToken).to.be.equal(landscapeToken[landscape1].jwt);
            (0, chai_1.expect)(args.removed.length).to.be.equal(0);
            (0, chai_1.expect)(args.changed.length).to.be.equal(0);
        });
        it("checkForUpdates, scope exists, remove scope", async () => {
            ...
            (0, chai_1.expect)(args.added.length).to.be.equal(0);
            (0, chai_1.expect)(args.removed.length).to.be.equal(1);
            (0, chai_1.expect)(args.removed[0].accessToken).to.be.equal(objToken[landscape2].jwt);
            (0, chai_1.expect)(args.changed.length).to.be.equal(0);
        });
        it("checkForUpdates, scope exists, changed scope", async () => {
            ...
            (0, chai_1.expect)(args.changed[0].accessToken).to.be.equal(landscapeToken[landscape2].jwt);
        });
        it("checkForUpdates, previousToken exists but both jwt and iasjwt are falsy, should add session", async () => {
            ...
            (0, chai_1.expect)(await instance["checkForUpdates"]([])).to.be.undefined;
            const args = stubEmitter.args[0][0];
            (0, chai_1.expect)(args.added.length).to.equal(1);
            (0, chai_1.expect)(args.added[0].accessToken).to.equal("some-token");
            (0, chai_1.expect)(args.removed.length).to.equal(0);
            (0, chai_1.expect)(args.changed.length).to.equal(0);
        });
        it("checkForUpdates, previousToken exists with iasjwt, but session does not exist (should remove)", async () => {
            ...
            (0, chai_1.expect)(await instance["checkForUpdates"]([])).to.be.undefined;
            const args = stubEmitter.args[0][0];
            (0, chai_1.expect)(args.added.length).to.equal(0);
            (0, chai_1.expect)(args.removed.length).to.equal(1);
            (0, chai_1.expect)(args.removed[0].accessToken).to.equal(""); // jwt is empty string
            (0, chai_1.expect)(args.removed[0].iasToken).to.equal("some-iasjwt");
            (0, chai_1.expect)(args.changed.length).to.equal(0);
        });
    });
