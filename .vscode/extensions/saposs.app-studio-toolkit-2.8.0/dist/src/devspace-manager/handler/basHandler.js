...
function getParamFromUrl(query, name, optional = false) {
    const value = new node_url_1.URLSearchParams(query).get(name);
    if (!optional && !value) {
...
function handleLogin(uri) {
    ...
    exports.eventEmitter.fire({ jwt, iasjwt });
    (0, logger_1.getLogger)().info("jwt(s) received");
}
