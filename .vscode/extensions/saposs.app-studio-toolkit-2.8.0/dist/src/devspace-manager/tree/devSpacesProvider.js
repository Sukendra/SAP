    ...
    async getTreeTopLevelChildren() {
        ...
        const rootNodes = (0, lodash_1.map)(landscapes, (landscape) => {
            const tooltip = landscape.isLoggedIn
                ...
                : messages_1.messages.lbl_not_logged_in;
            return new treeItems_1.LandscapeNode(this.extensionPath, landscape.name, vscode_1.TreeItemCollapsibleState.Expanded, (0, treeItems_1.getSvgIconPath)(this.extensionPath, `landscape`), "", landscape.default ? `${tooltip} ${messages_1.messages.lbl_ai_enabled}` : tooltip, landscape.name, landscape.url, messages_1.messages.lbl_landscape_context_status(landscape.isLoggedIn, landscape.default));
        });
        return rootNodes;
    }
}
